...
//[XiaoHe multicore] in start.S
/*__asm (".section .text");
__asm (".global _start");
__asm ("_start:");*/

// Enable Caches in MRAC
/* CSR: 0x7C0 - 0x7FF未分配 自定义 */  
//[XiaoHe multicore] No need
/*__asm ("li t0, 0x5f555555");
__asm ("csrw 0x7c0, t0");
__asm ("csrr t0, mhartid");
__asm ("bnez t0, setup_stack1");
__asm ("csrwi 0x7fc,3"); // enable hart1

// Set stack pointer.
__asm ("la sp, STACK-0x1000");
...
ee_u8 static_memblk0[TOTAL_DATA_SIZE];
ee_u8 static_memblk1[TOTAL_DATA_SIZE];
#endif
...
void
riscvPause(unsigned n)
{
  __asm volatile ("csrw 0x7c2, %[op0]" : : [op0] "r" (n) : );
}
...
MAIN_RETURN_TYPE main(int argc, char *argv[]) {
        ...
        ee_u8* static_memblk = static_memblk0;
...
#error "Please define a way to initialize a memory block."
        ...
        __asm("__perf_end:");
        if(hartId == 1) {
            //riscvPause(100000); // keep it here to let print and finish by hart0
            while(1) {;}  // keep it here to let print and finish by hart0
        }
...
//AG: patch:
default_num_contexts = 2;
        //int int_part = 1000*default_num_contexts*results[0].iterations/time_in_secs(total_time);
        int int_part = 40000/time_in_secs(total_time);
        //int decimal_part = 100000*default_num_contexts*results[0].iterations/time_in_secs(total_time) % 100;
        int decimal_part = 4000000/time_in_secs(total_time) % 100;
        if (time_in_secs(total_time) > 0) {
                //ee_printf("Iterat/Sec/MHz   : %d.%02d\n",int_part, decimal_part);
                ee_printf("Iterat/Sec/MHz   : %d",int_part);
                ee_printf(".%02d\n", decimal_part);
        }
#endif
